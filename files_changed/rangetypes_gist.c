...
/* Minimum share required to force upper-infinite class split (25%). */
#define UPPER_INF_SPLIT_MIN_FRACTION_NUM 1
#define UPPER_INF_SPLIT_MIN_FRACTION_DEN 4
...
range_gist_picksplit(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Current-row-aware split: if upper-infinite ranges form a meaningful and
	 * balanced subset, separate them from the rest.
	 */
	upper_inf_count = count_in_classes[CLS_UPPER_INF] +
		count_in_classes[CLS_UPPER_INF | CLS_CONTAIN_EMPTY];
	other_count = total_count - upper_inf_count;

	if (upper_inf_count > 0 && other_count > 0 &&
		upper_inf_count * UPPER_INF_SPLIT_MIN_FRACTION_DEN >=
		total_count * UPPER_INF_SPLIT_MIN_FRACTION_NUM &&
		other_count * UPPER_INF_SPLIT_MIN_FRACTION_DEN >=
		total_count * UPPER_INF_SPLIT_MIN_FRACTION_NUM)
	{
		SplitLR		classes_groups[CLS_COUNT];

		memset(classes_groups, 0, sizeof(classes_groups));
		classes_groups[CLS_UPPER_INF] = SPLIT_LEFT;
		classes_groups[CLS_UPPER_INF | CLS_CONTAIN_EMPTY] = SPLIT_LEFT;
